...
int part2(char *line)
{
    ...
    {
        if (i % 2 == 0){
            update_coords(&coord_robot_santa, line[i]);
        } else {
            update_coords(&coord_santa, line[i]);
        }

        if (is_coord_visited(coord_houses, count_visited, coord_santa) == 0 || 
            is_coord_visited(coord_houses, count_visited, coord_robot_santa) == 0)
        {
            if (i % 2 == 0){
                coord_houses[count_visited] = coord_robot_santa;
            } else {
...
{	
    ...
    printf("year 2015 day03 part 2 : %d \n", part2(line));
	return 0;
}